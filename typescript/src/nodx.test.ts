...
describe('el', () => {
  ...
  it('Element with nested elements', () => {
    const element = el('ul', el('li', text('Item 1')), el('li', text('Item 2')))
    const expected = '<ul><li>Item 1</li><li>Item 2</li></ul>'
    expect(element.render()).toBe(expected)
  })
...
describe('elVoid', () => {
  ...
  it('Void element with attributes', () => {
    const element = elVoid('img', attr('src', 'image.png'), attr('alt', 'An image'))
...
describe('group', () => {
  it('Groups multiple nodes', () => {
    const grouped = group(text('Hello, '), el('strong', text('World')), text('!'))
...
describe('classx', () => {
  ...
  it('Classes from object', () => {
    const classAttribute = classx({ class1: true, class2: false, class3: true })
    const expected = ' class="class1 class3"'
    expect(classAttribute.render()).toBe(expected)
  })

  it('Mixed class types', () => {
    const classAttribute = classx('class1', { class2: true, class3: false }, 'class4')
  ...
  it('Nested objects (should ignore nested objects)', () => {
    const classAttribute = classx({ class1: true, nested: { class2: true } } as any)
    const expected = ' class="class1"'
    expect(classAttribute.render()).toBe(expected)
  })
...
describe('NodeElement Edge Cases', () => {
  ...
  it('Void element with attributes and invalid children', () => {
    const tag = 'input'
    const n = new NodeElement(true, tag, attr('type', 'text'), null as any, 456 as any)
  ...
  it('Element with duplicated attribute names', () => {
    ...
    const expected = '<div data-test="value1" data-test="value2" data-test="value3"></div>'
    expect(n.render()).toBe(expected)
  })
})
...
describe('Combined Helpers', () => {
  ...
  it('Complex nesting and helper functions', () => {
    ...
    const nav = el(
      'nav',
      el('ul', mapx(['Home', 'About', 'Contact'], (page) =>
        el('li', createLink(`/${page.toLowerCase()}`, page))
      ))
    )
    const expected =
